    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        if (builder.requestFlag) {
            if (request.method() == "GET") {
                ...
                io.merculet.core.api.interceptor.Logger.printJsonRequest(builder, request)
            } else {

                if (subtypeIsNotFile(rSubtype)) {
                    io.merculet.core.api.interceptor.Logger.printJsonRequest(builder, request)
                } else {
                    io.merculet.core.api.interceptor.Logger.printFileRequest(builder, request)
                }
            }
        }
        ...
        if (builder.responseFlag) {
            ...
            if (subtypeIsNotFile(subtype)) {
                val bodyString = io.merculet.core.api.interceptor.Logger.getJsonString(responseBody.string())
                io.merculet.core.api.interceptor.Logger.printJsonResponse(builder, chainMs, isSuccessful, code, header, bodyString, segmentList)
                body = ResponseBody.create(contentType, bodyString)
            } else {
                io.merculet.core.api.interceptor.Logger.printFileResponse(builder, chainMs, isSuccessful, code, header, segmentList)
                return response
            }
